 ...
 */
final class Version20220905131412 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE VIEW view_official_team_ranking AS 
                        SELECT
                team_id as id,team_id,
                official_team.position as overall_position,
                if (race.date > "2021-08-31 23:59:59",
                    (SELECT GREATEST(LEAST(150,count(*)) - SUM(CASE WHEN (official_team.position > other_team.position) THEN 1 ELSE 0 END), 1) FROM view_team AS other_team where other_team.race_id = official_team.race_id and other_team.gender = official_team.gender),
                    scale.value) as points,
                official_team.race_id,
                if (race.date > "2021-08-31 23:59:59",
                (SELECT count(*)+1 FROM view_team AS other_team
                 WHERE overall_position > other_team.position
                   AND other_team.race_id = official_team.race_id
                   AND other_team.gender = official_team.gender)
            ...
            FROM view_official_team as official_team
                     JOIN race ON race.id = official_team.race_id
