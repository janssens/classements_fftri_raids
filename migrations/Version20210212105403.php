 ...
 */
final class Version20210212105403 extends AbstractMigration
{
    ...
    {
        $this->addSql('DROP VIEW view_official_unisex_ranking');
        $this->addSql('CREATE VIEW view_official_unisex_ranking AS 
        SELECT CONCAT(LPAD(athlete_id,6,"0"),LPAD(championship.id,3,"0")) as id, athlete_id,sum(race.coefficient*value) as points,championship.id as championship_id
        FROM view_official_team AS team
         ...
         GROUP BY athlete_id,championship.id;
            ');
    }
    ...
    public function down(Schema $schema) : void
    {
        $this->addSql('DROP VIEW view_official_unisex_ranking');
        $this->addSql('CREATE VIEW view_official_unisex_ranking AS 
        SELECT CONCAT(LPAD(athlete_id,6,"0"),LPAD(championship.id,3,"0")) as id, athlete_id,sum(value) as points,championship.id as championship_id
        FROM view_official_team AS team
         JOIN registration_team AS rt ON rt.team_id = team.team_id
         JOIN registration AS r ON r.id = rt.registration_id
         JOIN athlete AS a ON a.id = r.athlete_id
